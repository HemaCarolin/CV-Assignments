...
class Extractor(object):
	...
	def extract_keypoints(self, img):
		...
		# extraction
		if pts is not None:
			kpts = [cv2.KeyPoint(x=p[0][0], y=p[0][1], size=50, angle=-1, response=0, octave=0, class_id=-1) for p in pts]
		else:
			kpts = []
		...
		if self.last is not None:
			...
			coords1_match_pts = np.asarray([kpts[m.queryIdx].pt for m,n in matches], dtype=np.float32)
			coords2_match_pts = np.asarray([self.last["kpts"][m.trainIdx].pt for m,n in matches], dtype=np.float32)
